...
public:
    ...
    Timestamp pollReturnTime() const {return pollReturnTime_;}//记录上次 poll() 返回的时间。
...
private:
    ...
    std::unique_ptr<Channel> wakeupChannel_;//专门监听 wakeupFd_ 的 Channel。一旦有写入，触发 handleRead

    ChannelList activeChannels_;//存储本次 poll() 返回的所有发生事件的 Channel 指针。
