...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    // 说明当前这一次write并没有把数据全部发送出去，剩余的数据需要保存到缓冲区当中，然后给channel
    //注册epollout事件，poller发现tcp的发送缓冲区有空间，会通知相应的 sock->channel 调用 handleWrite 回调方法
    //也就是调用TcpConnection::handleWrite方法，把发送缓冲区中的数据全部发送完成
    if(!faulteError && remaining > 0)
    {
        //目前发送缓冲区剩余的待发送数据的长度
...
{
    LOG_INFO("Tcpconnection::handelClose fd=%d state=%d \n", channel_->fd(), (int)state_);
    setState(kDisconnected);
    channel_->disableAll();
